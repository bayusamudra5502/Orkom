 ...
 *   Rating: 1
 */
int amber(int x, int y) { return ~(~x & y) & ~(x & ~y); }
/*
 ...
 *   Rating: 2
 */
int dvalin(int x) { return (((1 << 31) >> 31) ^ x) + 1; }
/*
 ...
 *   Rating: 2
 */
int handbook(int x, int n) { return (x >> (n << 3)) & 255; }
/*
 ...
 *   Rating: 4
 */
int medaka(int x) { return ((x >> 31) ^ x) + ((x >> 31) & 1); }
/*
 ...
 */
int sayu(int x) {
  return (x << 24) | ((x << 8) & (255 << 16)) | ((x >> 8) & (255 << 8)) |
         ((x >> 24) & 255);
}
/*
 ...
 *  Rating: 3
 */
int azhdaha(int x) { return ((x << 1) + x); }
/*
