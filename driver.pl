...
BEGIN {
  $GRADER_PATH = $ENV{GRADER_PATH};
  ($GRADER_PATH ne '')
    or die "$0: env. GRADER_PATH is not defined\n";
}
...
if ($USE_BTEST) {
    ...
    unless (system("cd $GRADER_PATH; cp -r $driverfiles $tmpdir") == 0) {
	clean($tmpdir);
...
#
print "Identify coding rules violations...\n";
...
#
print "Determining correctenss score...\n";
if ($USE_BTEST) {
    system("cp zap-bits.c bits.c");

    # Compile btest
    system("make btestexplicit > /dev/null 2>&1") == 0
...
# 
print "Identifying operator count violations...\n";
...
#
print "Determining performance score...\n";
if ($USE_BTEST) {
    system("cp Zap-bits.c bits.c");

    # Compile btest
    system("make btestexplicit verbose=0 > /dev/null 2>&1") == 0
	or die "$0: Could not make btest in $tmpdir. $diemsg\n";
    print "\n";
...
#
print "Getting operator count of each function...\n";
...
if ($nickname) {
    ...
    &Driverlib::source_upload($nickname);
    &Driverlib::driver_post("$nickname", $autoresult, $autograded, "$user_nickname");
}
